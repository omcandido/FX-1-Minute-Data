...
def download_all():
    ...
    if DT_EXISTS:
        print(f"Delta table exists at {output_folder}")
        dt = DeltaTable(output_folder)
        partitions = pd.DataFrame(dt.partitions())
        pairs = partitions.pair.unique()
    else:
        print(f"Delta table does not exist at {output_folder}")
    ...
    with open('pairs.csv', 'r') as f:
        ...
        for row in reader:
            if DT_EXISTS:
                pair = row[1].upper()
                if pair in pairs:
                    print(f"Pair {pair} already exists in Delta Lake, trying to update...")
