...
def download_all():
    with open('pairs.csv', 'r') as f:
        ...
        for row in reader:
            ...
            print('Downloading data for', currency_pair_name)
            ...
            try:
                while True:
                    ...
                    try:
                        print('Downloading data for', currency_pair_name, 'for year', year)
                        _ = download_hist_data(year=year,
                                                      pair=pair,
                    ...
                    while not could_download_full_year and month <= 12:
                        print('Downloading data for', currency_pair_name, 'for year', year, 'and month', month)
                        _ = download_hist_data(year=str(year),
                                                      month=str(month),
                                                      pair=pair,
                                                      output_directory=output_folder,
                                                      verbose=False)
